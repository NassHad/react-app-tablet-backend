// Exact match only import script for wipers products with detailed logging
...
async function importWipersProductsExactOnly() {
  try {
    console.log('🚀 Starting exact match only wipers products import...');
    ...
    console.log('\n🎉 Exact match only wipers products import completed!');
    console.log(`📊 Summary:`);
    ...
    if (problems.missingBrands.length > 0) {
      console.log('\n❌ Missing Brands:');
      problems.missingBrands.forEach(brand => {
        console.log(`   - "${brand.brand}" (slug: ${brand.slug}) - ${brand.modelsCount} models`);
      });
    }
    
    // Log missing models (first 10)
    if (problems.missingModels.length > 0) {
      console.log('\n❌ Missing Models (first 10):');
      problems.missingModels.slice(0, 10).forEach(model => {
        console.log(`   - Brand: "${model.brand}" → Model: "${model.model}"`);
        console.log(`     Key: ${model.key}`);
        console.log(`     Has wipers data: ${Object.keys(model.wipersData).length > 0 ? 'Yes' : 'No'}`);
      });
      
      if (problems.missingModels.length > 10) {
        console.log(`   ... and ${problems.missingModels.length - 10} more missing models`);
      }
    }
    ...
    // Log skipped models summary
    if (problems.skippedModels.length > 0) {
      console.log('\n⚠️  Skipped Models Summary:');
      const skipReasons = {};
      problems.skippedModels.forEach(skip => {
        skipReasons[skip.type] = (skipReasons[skip.type] || 0) + 1;
      });
      
      Object.keys(skipReasons).forEach(reason => {
        console.log(`   - ${reason}: ${skipReasons[reason]}`);
      });
    }
    
    // Save problems to a file for later analysis
    const problemsPath = path.join(process.cwd(), 'scripts', 'wipers-import-problems.json');
