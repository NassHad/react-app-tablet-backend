// Re-import models with proper brand relationships
const fs = require("fs");
...
async function reimportModelsWithRelationships() {
  try {
    console.log('🔄 Re-importing models with proper brand relationships...');
    ...
    // Now re-import models with proper relationships
    let created = 0;
    let skipped = 0;
    let errors = 0;
    ...
    for (let i = 0; i < modelsData.length; i++) {
      ...
      try {
        ...
        // Create model with proper relationship
        const data = {
          data: {
            slug: item.modelSlug,
            name: item.name,
            brand: brandId, // This should work with the current schema
            isActive: true
          }
        };
...
reimportModelsWithRelationships();
