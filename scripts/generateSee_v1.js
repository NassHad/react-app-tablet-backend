...
async function insertData(db, syncData) {
  ...
  // Helper function pour extraire l'URL d'un objet Media Strapi et convertir en chemin relatif
  const extractMediaPath = (media, id, category, type) => {
    if (!media) return null;
    
    // Si c'est déjà un chemin relatif (après téléchargement)
    if (typeof media === 'string' && media.startsWith('images/')) {
      return media;
    }
    
    // Si c'est un objet Media Strapi, extraire l'URL et convertir en chemin
    if (typeof media === 'object') {
      const url = media.formats?.small?.url || media.url || media.formats?.thumbnail?.url;
      if (url) {
        // Convertir l'URL en chemin relatif pour l'APK
        // Ex: http://strapi-server/uploads/light_123_img.jpg -> images/lights/123_img.jpg
        return `images/${category}/${id}_${type}.jpg`;
      }
    }
        ...
        tables.forEach(strapiTableName => {
          ...
          data.forEach((record, index) => {
            ...
            const values = strapiColumns.map(strapiCol => {
              ...
              // NOUVEAU: Extraire les chemins d'images pour img et brandImg
              if ((strapiCol === 'img' || strapiCol === 'brandImg') && value) {
                const category = sqliteTableName === 'light_data' ? 'lights'
                               : sqliteTableName === 'battery_data' ? 'battery'
                               : sqliteTableName === 'wipers_data' ? 'wipers'
                               : sqliteTableName === 'filter_products' ? 'filters'
                               : null;
                
                if (category) {
                  const type = strapiCol === 'img' ? 'img' : 'brand';
                  value = extractMediaPath(value, record.id, category, type);
                }
              }
              
              // Convert objects to JSON strings (sauf pour les images déjà traitées)
              if (typeof value === 'object' && value !== null && !(strapiCol === 'img' || strapiCol === 'brandImg')) {
