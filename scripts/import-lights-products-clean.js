// Import Lights Products with Clean Data - Use the cleaned osram data
...
function mapSlugToDatabase(brandSlug, modelSlug) {
  ...
  const brandMappings = {
    ...
    'daewoo': 'chevrolet',
    'citron': 'citroen'  // Fix corrupted CITROEN character
  };
...
(async () => {
  try {
    console.log('🚀 Starting Lights Products import with CLEAN data...');
    
    // Read the CLEANED data file
    const dataPath = path.join(process.cwd(), 'scripts', 'osram_bulbs_cleaned.json');
    const rawData = fs.readFileSync(dataPath, 'utf8');
    const data = JSON.parse(rawData);
    
    console.log(`📊 Found ${data.length} CLEAN records in osram_bulbs_cleaned.json`);
