 ...
 *
 * Exports all brands and models from the Strapi database into structured JSON files:
 * - One file per brand containing all its models
 * - Models without brand relationships batched into 1000-model files
 * - Summary file with all brands
 *
...
(async () => {
  ...
  try {
    ...
    if (!DRY_RUN) {
      console.log(`✅ Exported: all-brands.json\n`);
    }
    ...
    // Process brands in batches for parallel writes
    const brandBatches = chunk(allBrands, PARALLEL_WRITES);

    for (const batch of brandBatches) {
      await Promise.all(batch.map(async (brand, batchIndex) => {
        try {
