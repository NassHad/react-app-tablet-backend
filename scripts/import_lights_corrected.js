// Corrected Lights Import Script for Strapi Console
// Copy and paste this ENTIRE script into your Strapi console

(async () => {
  console.log('🚀 Starting Corrected Osram Lights Import...');
  ...
  try {
    ...
    // Step 3: Create unique positions (shared across all models)
    ...
    // Step 5: Link models to positions (many-to-many relationship)
    console.log('📝 Step 4: Linking models to positions...');
    ...
    for (const modelPositionKey of uniqueModelPositions) {
      ...
      try {
        // Update the model to include this position
        ...
        linkSuccessCount++;
        if (linkSuccessCount % 100 === 0) {
          console.log(`📊 Linked ${linkSuccessCount} model-position combinations...`);
        }
      } catch (error) {
        linkErrorCount++;
        console.error(`❌ Error linking ${modelName} to ${positionName}:`, error.message);
      }
    }

    console.log(`📊 Model-Position Links: ${linkSuccessCount} created, ${linkErrorCount} errors`);
